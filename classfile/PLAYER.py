...
class _PLAYER(pg.sprite.Sprite):
    def __init__(self):
        ...
        players.add(self);      all_sprites.add(self)
    ...
    def update(self):
        ...
        if keypressed[self.key_right] and self.rect.right < self.right_limit:   
            ...
            self.frame = 2
        if keypressed[self.key_up] and self.rect.top > self.top_limit:          
            self.rect.y -= self.speed   # ? UP
        if keypressed[self.key_down] and self.rect.bottom < self.bottom_limit:  
